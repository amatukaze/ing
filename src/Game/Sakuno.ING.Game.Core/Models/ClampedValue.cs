...
public readonly struct ClampedValue : IEquatable<ClampedValue>
{
    ...
    public static ClampedValue operator +(ClampedValue clamped, int value) =>
        new(Math.Min(clamped.Max, clamped.Current + value), clamped.Max);
    public static ClampedValue operator -(ClampedValue clamped, int value) =>
        new(Math.Max(0, clamped.Current - value), clamped.Max);

    public static implicit operator ClampedValue((int current, int max) tuple) =>
        new(tuple.current, tuple.max);

    public static ClampedValue FromRemaining(int max, int remaining) => new(max - remaining, max);
